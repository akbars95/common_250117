...
COMMENT ON COLUMN T_USERS.ADD_USER_DATE_TIME IS 'This column store user date and time when user registred';
...
CREATE TABLE T_PERSISTENT_LOGINS(
  ACCOUNT_USER_ID NUMBER(10,0) NOT NULL,
  SERIES VARCHAR2(128) NOT NULL,
  TOKEN VARCHAR2(128) NOT NULL,
  LAST_USED TIMESTAMP NOT NULL,
  CONSTRAINT PL_ACCOUNT_USER_ID_FK FOREIGN KEY (ACCOUNT_USER_ID) REFERENCES T_ACCOUNTS(ACCOUNT_USER_ID)
);
COMMENT ON TABLE T_PERSISTENT_LOGINS IS 'This table store user remember me tokens';
COMMENT ON COLUMN T_PERSISTENT_LOGINS.ACCOUNT_USER_ID IS 'This column store user account id';
COMMENT ON COLUMN T_PERSISTENT_LOGINS.SERIES IS 'This column store series';
COMMENT ON COLUMN T_PERSISTENT_LOGINS.TOKEN IS 'This column store token';
COMMENT ON COLUMN T_PERSISTENT_LOGINS.LAST_USED IS 'This column store user last used time stamp';

/*
CREATE TABLE COPY_ACCOUNTS(
  USER_ACCOUNT_COPY_ID NUMBER(10,0) DEFAULT SEQ_USER_ACCOUNT_COPY.NEXTVAL NOT NULL,
  ACCOUNT_USER_ID NUMBER(10,0) NOT NULL,
  LAST_COPY_DATE_TIME TIMESTAMP NOT NULL,
  CONSTRAINT USER_ACCOUNT_COPY_ID_FK PRIMARY KEY (USER_ACCOUNT_COPY_ID),
  CONSTRAINT AC_ACCOUNT_USER_ID_FK FOREIGN KEY (ACCOUNT_USER_ID) REFERENCES T_ACCOUNTS(ACCOUNT_USER_ID)
);*/
